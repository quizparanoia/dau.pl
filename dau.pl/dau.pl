################################################################################
# $Id: dau.pl 291 2009-09-06 18:10:29Z heidinger $
################################################################################
#
...
# Workaround for MacIrssi

our $module_statusbar_found = 1;

unless (eval "require Irssi::TextUI; 1") {
	print "Perl module Irssi::TextUI not found. dau.pl will work but the statusbar item will not be available.";
	$module_statusbar_found = 0;
}
...
$VERSION = '2.5.0';
#$VERSION = '2.5.0 SVN ($LastChangedRevision: 291 $)';
%IRSSI = (
          authors     => 'Clemens Heidinger',
          changed     => '$LastChangedDate: 2009-09-06 20:10:29 +0200 (So, 06 Sep 2009) $',
          commands    => 'dau',
...
Irssi::settings_add_bool('misc', 'dau_babble_options_noat', 0);
...
%9--babble%9
    ...
    %9-noat%9 %Uon|off%U:
        Only babble something not directed at anyone

    %9-options_line_by_line%9 %Ustring%U:
        Overrides setting dau_babble_options_line_by_line

    %9-options_preprocessing%9 %Ustring%U:
        Overrides setting dau_babble_options_preprocessing
...
%9--substitute%9
    ...
    %9-file%9 %Ustring%U:
        Override setting dau_files_substitute
...
our %switches = (
    ...
    nocombo => {
                ...
                babble       => {
                                 ...
                                 options_line_by_line  => { '*' => 1 },
                                 options_preprocessing => { '*' => 1 },
                                },
...
sub switch_babble {
	...
	my $options_line_by_line  = return_option('babble', 'options_line_by_line', $option{dau_babble_options_line_by_line});
	my $options_preprocessing = return_option('babble', 'options_preprocessing', $option{dau_babble_options_preprocessing});
	...
	my $option_babble_noat         = return_option('babble', 'noat');
	...
	if ($option_babble_at) {
		...
		$text = &babble_get_text($channel, \@filter, \@nicks, $option_babble_history_size, $options_line_by_line, $options_preprocessing, $option_babble_noat);
	} else {
		$text = &babble_get_text($channel, \@filter, undef, $option_babble_history_size, $options_line_by_line, $options_preprocessing, $option_babble_noat);
	}
	...
	babble_start($channel, $text, 0, $options_line_by_line);
...
sub switch_changelog {
	...
	2009-02-22    release 2.4.4
	              maintenance release

	2009-06-01    release 2.5.0
	              - new switch for %9--substitute%9: %9-file%9
	              - new switch for %9--babble%9: %9-options_line_by_line%9
	              - new switch for %9--babble%9: %9-options_preprocessing%9

	CURRENT
	              - new switch for %9--babble%9: %9-noat%9
	              - new setting: %9dau_babble_options_noat%9
	END
...
sub switch_help {
	...
	elsif ($option_setting eq 'dau_babble_options_noat') {
		$output = &fix(<<"		END");
		%9dau_babble_options_noat%9 %Ubool

		Specifies whether timed babbles use the -noat option or not.
		END
	}
...
sub timer_babble_writing_reset {
	...
	if ($babble{line} =~ s/^BABBLE_INTERVAL=(\d+)\x02//) {
		$interval = $1;
		$babble{line} = parse_text("$babble{options_line_by_line} $babble{line}");
...
sub timer_remote_babble {
	...
	# else get text from file
		...
		my @filter = ();
		if ($option{dau_babble_options_noat}) {
			$text = &babble_get_text($channel, \@filter, undef, $option{dau_babble_history_size}, $option{dau_babble_options_line_by_line}, $option{dau_babble_options_preprocessing}, 'on');
		} else {
			$text = &babble_get_text($channel, \@filter, undef, $option{dau_babble_history_size}, $option{dau_babble_options_line_by_line}, $option{dau_babble_options_preprocessing}, 'off');
		}
	...
	babble_start($channel, $text, 1, $option{dau_babble_options_line_by_line});
}
...
sub babble_get_text {
	my ($channel, $filter, $nicks, $history_size, $options_line_by_line, $options_preprocessing, $options_noat) = @_;
	my $output;
	...
	foreach my $line (@file) {
		...
		if ($add) {
			...
			if ((lc($options_noat) eq 'on') && ($minimum_number_nicks > 0)) {
				$add = 0;
			}
		}
		...
		if ($add) {
			if ($line =~ /$compiled_pattern_ops/) {
				...
				if ((lc($options_noat) eq 'on') && ($minimum_number_ops > 0)) {
					$add = 0;
				}
			}
		}
	...
	if ($options_preprocessing !~ /^\s*$/) {
		$text = parse_text("$options_preprocessing \x02$text");
		$text =~ s/^\x02//;
	}
	...
	while ($text =~ /(.*?)(\n|$)/g) {
		...
		my $newtext = parse_text("$options_line_by_line $line") . "\n";
...
sub babble_start {
	my ($channel_rec, $text, $remote, $options_line_by_line) = @_;
	...
	$babble{options_line_by_line} = $options_line_by_line;
...
sub set_settings {
	...
	$option{dau_babble_options_noat}             = Irssi::settings_get_bool('dau_babble_options_noat');
